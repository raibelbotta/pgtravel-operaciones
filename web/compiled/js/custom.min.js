...
else{e.add(a,'days');n.find('form input.datepicker:last').val(e.format('DD/MM/YYYY'))};i()});a.initDatepickers(n.find('input.datepicker:first').parent(),n.find('input.datepicker:last').parent(),i);n.find('select').on('change',function(){i()});e.find('a[role=tab][aria-controls=tab-hotel]').on('shown.bs.tab',function(){i()})},i=function(){var n=e.find('#tab-private-house'),r=n.find('table.table-results');r.dataTable({serverSide:!0,ajax:{url:Routing.generate('app_offers_getprivatehouseprices'),data:function(e){var a=[];t.each(t('form#filter-private-house').serializeArray(),function(t,e){a[e['name']]=e['value']});return t.extend(!0,e,a)},method:'POST'},columns:[{name:'supplier',title:Translator.trans('Supplier')},{name:'room',title:Translator.trans('Room')},{name:'plan',title:Translator.trans('Plan')},{name:'price',title:Translator.trans('Price')},{name:'total',title:Translator.trans('Total'),sortable:!1},{sortable:!1,searchable:!1,width:'40px'}]});var i=function(){r.dataTable().api().draw()};n.find('form .date:first').on('dp.change',function(t){var e=t.date?t.date.clone():null,a=n.find('form input.datepicker:last').val()?moment(n.find('form input.datepicker:last').val(),'DD/MM/YYYY'):null;if(!e||!a){n.find('form input[name$="[nights]"]').val('')}
