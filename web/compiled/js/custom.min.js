...
App=typeof App!=="undefined"?App:{};App.Suppliers=typeof App.Suppliers!=="undefined"?App.Suppliers:{};+(App.Suppliers.Form=function(c){var b=function(){App.Forms.initTelephoneControl(c("form#supplier input[type=tel]"));c("form#supplier .collection-employees").on("item-added.app",function(d,e){App.Forms.initTelephoneControl(c(e.item).find("input[type=tel]"))});c('form#supplier select[name$="[place]"]').select2({width:"100%",minimunInputLength:1,ajax:{url:Routing.generate("app_suppliers_getplaces"),dataType:"json",delay:250,method:"GET",processResults:function(d){var e=d.data;e.unshift({id:0,text:""});return{results:e}}}})};var a=function(){c("#supplier").validate({errorPlacement:function(d,e){if(!e.data("tooltipster-ns")){e.tooltipster({trigger:"custom",onlyOne:false,position:"bottom-left",positionTracker:true})}e.tooltipster("update",c(d).text());e.tooltipster("show")},success:function(d,e){c(e).tooltipster("hide")}})};return{init:function(){b();a()}}}(jQuery));
