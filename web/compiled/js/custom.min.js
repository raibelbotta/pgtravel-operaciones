...
else{r()}};n.find('form input:checkbox').iCheck({checkboxClass:'icheckbox_flat-green'});n.find('form input:checkbox').on('ifChanged',function(){i()});a.initDatepickers(n.find('input.datepicker:first').parent(),n.find('input.datepicker:last').parent(),i);n.find('form#filter-transport').find('input:not(.datepicker), select').on('change',function(){i()});e.find('a[role=tab][aria-controls=tab-transport]').on('shown.bs.tab',function(){i()});n.on('click','button.btn-select-service',function(){var i=t(this).data('service'),r=e.data('item');a.translateResults(r,{'model':'transport','startAt':n.find('input.datepicker:first').val()+' 08:00','endAt':n.find('input.datepicker:last').val()+' 08:00','name':i.name,'supplier':i.supplier.id,'cost':i.cost,'totalPrice':i.totalPrice});r.find('[name$="[supplier]"]').trigger('change.select2');r.find('[name$="[totalPrice]"]').trigger('change');e.modal('hide')})},l=function(){e.find('.tab-content .tab-pane:not(#tab-hotel, #tab-private-house, #tab-car-rental, #tab-transport)').each(function(){var n=t(this),i=n.attr('id').replace(/^tab\-/,''),r=function(){var t={'model':i,'from':n.find('input.datepicker:first').val(),'to':n.find('input.datepicker:last').val(),'quantity':n.find('select[name=quantity]').val()};n.find('.table-responsive.results').empty().text(Translator.trans('Loading services...')).load(Routing.generate('app_offers_getgeneralserviceprices'),t,function(){n.find('.table-responsive.results table').DataTable({})})};a.initDatepickers(n.find('.date:has(.datepicker):first'),n.find('.date:has(.datepicker):last'),r);n.find('form').find('select, input:not(.datepicker)').on('change',function(){r()});n.on('click','button.btn-select-service',function(){var r=t(this).data('service'),o=e.data('item');e.modal('hide');a.translateResults(o,{'model':i,'startAt':n.find('input.datepicker:first').val(),'endAt':n.find('input.datepicker:last').val(),'name':r.serviceName,'supplier':r.supplier.id,'pax':r.pax,'cost':r.cost,'totalPrice':r.totalPrice});o.find('[name$="[supplier]"]').trigger('change.select2');o.find('[name$="[totalPrice]"]').trigger('change')});e.find('a[role=tab][aria-controls=tab-'+i+']').on('show.bs.tab',function(){r()})})};n();i();r();o();l();+(function(){var t=e.data('item').find('select[name$="[model]"]').val();e.find('.nav.nav-tabs a[href="#tab-'+t+'"]').tab('show')}())};return{init:function(){e();n();i()}}}(jQuery));
