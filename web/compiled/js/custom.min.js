...
App=typeof App!=="undefined"?App:{};App.Bookings=typeof App.Bookings!=="undefined"?App.Bookings:{};App.Bookings.Form=function(){var b=function(){var d=function(h,j){var k=h.find('input[name$="[nights]"]'),l=h.find("input.datepicker"),i=$.now();if(k.data("ajax")){k.data("ajax").abort()}if($(l[0]).val()==""){return}if($(j).is(".datepicker")){if($(l[1]).val()==""){k.val("");return}var m=$.ajax(Routing.generate("app_offers_getnights")+"?id="+i,{data:{from:$(l[0]).val(),to:$(l[1]).val()},dataType:"json",method:"POST",success:function(n){k.val(n.nights);k.removeData("ajax")}})}else{if($(j).val()==""){$(l[1]).val("");return}var m=$.ajax(Routing.generate("app_offers_getnights")+"?id="+i,{data:{from:$(l[0]).val(),nights:k.val()},dataType:"json",method:"POST",success:function(n){$(l[1]).val(n.to);k.removeData("ajax")}})}k.data("ajax",m)};var f=function(j){var i=$(j).find("input.datepicker"),h={format:"DD/MM/YYYY HH:mm",showClear:true,showTodayButton:true};$(i[0]).parent().datetimepicker(h);$(i[1]).parent().datetimepicker($.extend({},h,{useCurrent:false}));$(i[0]).parent().on("dp.change",function(k){$(i[1]).parent().data("DateTimePicker").minDate(k.date);d($(j),i[0])});$(i[1]).parent().on("dp.change",function(k){$(i[0]).parent().data("DateTimePicker").maxDate(k.date);d($(j),i[1])})};var e=function(h){$(h).find('select[name$="[origin]"], select[name$="[destination]"]').select2({minimunInputLength:1,width:"100%",ajax:{url:Routing.generate("app_offers_getplaces"),dataType:"json",delay:250,method:"GET",data:function(i){return{q:i.term,page:i.page}},processResults:function(i){var j=i.data;j.unshift({id:0,text:""});return{results:j}}}})};var g=function(h){$(h).find('select[name$="[supplier]"]').select2({width:"100%"})};$(".item.item-service").each(function(){f(this);e(this);g(this)});$("body").on("change",'.item-service input[name$="[nights]"]',function(){d($(this).closest(".item"),this)});$("body").on("click",".btn-add-item",function(n){var m=$(this);if(m.is("a")){n.preventDefault()}var o=$(m.data("collection")),k=o.data("index"),j=o.data("prototype");$counter=o.data("counter")?$(o.data("counter")):null;$item=$(j.replace(/__name__/g,k));o.data("index",k+1);if($counter){$counter.val(parseInt($counter.val(),10)+1)}o.append($item);if($item.is(".item-service")){if(o.find(".item-service").length>1){var h=$item.prev();for(var l=0;l<2;l++){if(h.find(".datepicker:eq("+l+")").val()!==""){$item.find(".datepicker:eq("+l+")").val(h.find(".datepicker:eq("+l+")").val())}}}$item.find('select[name$="[model]"]').trigger("change");f($item);e($item);g($item)}$item.find("input:text:first").focus();if(o.closest("div.x_panel").find("h2").data("tooltipster-ns")){o.closest("div.x_panel").find("h2").tooltipster("hide")}$("body").animate({scrollTop:$item.offset().top},500)});$("body").on("click",".btn-remove-item",function(j){var i=$(this),h=i.closest(".item");if(i.is("a")){j.preventDefault()}swal({title:Translator.trans("Confirm operation"),text:Translator.trans("The service will be removed. Are you sure you want to continue?"),type:"warning",showCancelButton:true,confirmButtonColor:"#d9534f"},function(k){if(k){h.fadeOut(function(){var m=$(this).closest(".collection"),l=m.data("counter")?$(m.data("counter")):null;$(this).remove();if(l){l.val(l.val()-1)}})}})})};var a=function(){$("#reservation").validate({errorPlacement:function(d,e){if(e.is(":hidden")){e=e.closest(":visible")}if(!e.data("tooltipster-ns")){e.tooltipster({trigger:"custom",onlyOne:false,position:"bottom-left",positionTracker:true})}e.tooltipster("update",$(d).text());e.tooltipster("show")},ignore:':hidden:not(input[name="servicesCounter"])',messages:{servicesCounter:{min:Translator.trans("Add a service at least")}},rules:{servicesCounter:{min:1},"offer_form[directClientFullName]":{required:{depends:function(){return $("#offer_form_clientType_1").prop("checked")===true}}},"offer_form[client]":{required:{depends:function(){return $("#offer_form_clientType_0").prop("checked")===true}}},"offer_form[percentApplied][plus]":"number"},success:function(e,f){var d=$(f);if(d.is(":hidden")){d=d.closest(":visible")}d.tooltipster("hide")}})};var c=function(){$('input:radio[name="offer_form[clientType]"]').on("ifClicked",function(){var d=$(this).val();if(d==="direct"){$(".block-clienttype.block-clienttype-direct").show();$(".block-clienttype:not(.block-clienttype-direct)").find("input:text, select").each(function(){if($(this).data("tooltipster-ns")){$(this).tooltipster("hide")}});$(".block-clienttype:not(.block-clienttype-direct)").hide()}else{if(d==="registered"){$(".block-clienttype.block-clienttype-registered").show();$(".block-clienttype:not(.block-clienttype-registered)").find("input:text, select").each(function(){if($(this).data("tooltipster-ns")){$(this).tooltipster("hide")}});$(".block-clienttype:not(.block-clienttype-registered)").hide()}}});$("#offer_form_client").select2({width:"100%"});$("#offer_form_client").on("change",function(){$("#offer_form_notificationContact").empty();var d=$(this).val();$.getJSON(Routing.generate("app_offers_getclientcontacts")+"?client="+d,function(e){$("#offer_form_notificationContact").append($('<option value=""></option>'));$.each(e.elements,function(f,g){$("#offer_form_notificationContact").append($('<option value="'+g.id+'">'+g.text+"</option>"))})})});App.Forms.initTelephoneControl($("#offer_form_directClientMobilePhone"));$("body").on("click",".btn-search-service",function(){var d=$(this).closest(".item");$("#searchServiceModal").data("item",d).modal({backdrop:"static"});$("#searchServiceModal .modal-body").empty().append($("<p>"+Translator.trans("Loading data...")+"</p>")).load(Routing.generate("app_offers_searchservice"))});$("body").on("change",'.item.item-service select[name$="[model]"]',function(){var d=$(this).closest(".item"),f=this.options[this.selectedIndex];var e=$(f).data("options")?$(f).data("options").split(" "):[];$.each(e,function(g){var h=e[g];if(h.substr(0,1)==="-"){d.find(".visible-"+h.substr(1)).hide()}else{d.find(".visible-"+(h.substr(0,1)=="+"?h.substr(1):h)).show()}});d.find('input[name$="[cost]"]').trigger("change")});$('.item.item-service select[name$="[model]"]').trigger("change");+(function(h){var e=function(i){return isNaN(parseFloat(i))?0:parseFloat(i)};var g=function(o){var n=h(o),k=n.find('input[name$="[nights]"]'),p=n.find('input[name$="[pax]"]'),j=n.find('input[name$="[cost]"]'),l=n.find('input[name$="[totalPrice]"]'),q=k.is(":visible")?e(k.val()):0,i=p.is(":visible")?e(p.val()):0,m=q*e(j.val())+i*e(j.val());l.val(m.toFixed(2))};var d=function(){var i=new Number(0);h('.item-service input[name$="[totalPrice]"]').each(function(){i+=e(h(this).val())});h("#offer_form_totalExpenses").val(i.toFixed(2)).trigger("change")};h("#offer_form_services").on("change",'input[name$="[nights]"], input[name$="[pax]"], input[name$="[cost]"]',function(){g(h(this).closest(".item-service"))});h("#offer_form_services").on("change",'.item-service input[name$="[totalPrice]"]',function(){d()});h(".item-administrative-charge").on("change","input",function(){if(!h(this).is('[name$="[pax]"], [name$="[multiplier]"], [name$="[price]"]')){return}var i=h(this).closest(".item"),j=e(i.find('input[name$="[multiplier]"]').val())*e(i.find('input[name$="[pax]"]').val())*e(i.find('input[name$="[price]"]').val());h(this).closest(".item").find('input[name$="[total]"]').val(j.toFixed(2)).trigger("change")});var f=function(){var i=h("#offer_form_totalCharges"),k=h('.item-administrative-charge input[name$="[total]"]'),j=new Number(0);k.each(function(){j+=e(h(this).val())});i.val(j.toFixed(2)).trigger("change")};h(".item-administrative-charge").on("change",'input[name$="[total]"]',function(){f()});h("#offer_form_totalExpenses, #offer_form_totalCharges, #offer_form_percentApplied_percent, #offer_form_percentApplied_plus").on("change",function(){var i=h("#offer_form_totalExpenses, #offer_form_totalCharges, #offer_form_percentApplied_percent, #offer_form_percentApplied_plus");var l=e(h("#offer_form_totalExpenses").val()),j=e(h("#offer_form_totalCharges").val()),m=h("#offer_form_percentApplied_percent"),k;if(m.val()==="plus"){k=new Number(l+j+e(h("#offer_form_percentApplied_plus").val()))}else{k=new Number(l*(m.val()/100)+l+j)}h("#offer_form_clientCharge").val(k.toFixed(2))});h("#offer_form_totalExpenses").trigger("change");h("#offer_form_percentApplied_percent").on("change",function(){if(h(this).val()!=="plus"){h("#offer_form_percentApplied_plus").val(0)}});d();f()}(jQuery))};return{init:function(){c();b();a()}}}();
